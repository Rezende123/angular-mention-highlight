...
export class Tela3Component implements OnInit {
  ...
  initializeText(textComponets){
    if(textComponets.length != 0){
      ...
      for (let i = 0; listDataBase[i]; i++) this.historySelected[index + i] = Tela3Component.wordName = listDataBase[i].nome;
  ...
  monitoringInput() {
    const newWord = Tela3Component.wordName;
    if (newWord != '') {
  ...
  ModelItem(name) {
    Tela3Component.wordName = name.name;
    return name.name + " ";
  }
  ...
  insertText(index){
    ...
    for(let i = 0; objectList[i]; i++){
      ...
      Tela3Component.wordName = objectList[i].nome;
    }
    this. monitoringInput();
  }
